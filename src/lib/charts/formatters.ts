/**
 * Date formatters and day-start helpers for time-series charts.
 *
 ...
 *
 * @param ianaTimeZone - e.g. 'Australia/Brisbane'
 * @param displayInterval - '5m' | '30m' | '1h' | '1d' | '7d' | '1M' | '3M' | 'quarter' | 'season' | 'half' | 'fy' | '1y'
 */
 ...
 *
 * @param viewStart - Viewport start (ms)
 * @param viewEnd - Viewport end (ms)
 * @param ianaTimeZone - e.g. 'Australia/Brisbane'
 * @param timeZone - offset string, e.g. '+10:00'
 * @returns `formatKey` identifies the formatter (branch + zones) so callers
 *   can skip reassigning an equivalent `formatTick` — the branch rule lives
...
export function getPowerAxisTicks(
	...
	ianaTimeZone: string,
	timeZone: string
): { ticks: Date[]; formatTick: (d: any) => string; formatKey: string } {
	...
	if (!(spanHours > 0) || spanHours > 48) {
		...
		return {
			ticks: dayStarts,
			formatTick: (d: any) => formatDayMonth(d, ianaTimeZone),
 ...
 *
 * @param data - Visible series rows (phase anchor only)
 * @param viewStart - Viewport start (ms)
 * @param viewEnd - Viewport end (ms)
 * @param timeZone - offset string
 */
function viewportBucketStarts(
	data: any[],
	viewStart: number,
	viewEnd: number,
	displayInterval: string,
	...
	const starts: { time: number }[] = [];
	for (let t = first; t <= viewEnd; t += fixedMs) starts.push({ time: t });
	return starts;
}

interface TimeAxisMemo {
	policyKey?: string;
	policy?: ReturnType<typeof getTimeFormatPolicy>;
	tickSource?: 'energy' | 'power';
	energyKey?: string;
	powerFormatKey?: string;
	lastTicks?: Date[];
}

/**
 * Apply the standard time-series x-axis + tooltip formatting to a chart store.
 *
 ...
 * rather than a hole.
 *
 * @param store - ChartStore instance
 * @param opts.data - Visible series rows
 * @param opts.viewStart - Viewport start (ms)
 * @param opts.viewEnd - Viewport end (ms)
 * @param opts.ianaTimeZone - e.g. 'Australia/Brisbane'
 * @param opts.timeZone - offset string, e.g. '+10:00'
 * @param opts.isEnergy - Whether the chart is in energy (stepped) mode
 * @param opts.displayInterval - '5m' | '30m' | '1h' | '1d' | '7d' | '1M' | '3M' | 'quarter' | 'season' | 'half' | 'fy' | '1y'
 */
export function applyTimeAxis(
	...
	perfSpan('chart:time-axis', () => {
		...
		if (memo.policyKey !== policyKey) {
			...
			store.formatTooltipX = (d: any) => policy.formatTooltip(d);
		}
		const policy = memo.policy!;
 ...
 */
const timeAxisMemos: WeakMap<object, TimeAxisMemo> = new WeakMap();
 ...
 */
function sameTickDates(a: Date[] | undefined, b: Date[]): boolean {
	if (a === b) return true;
...
/**
 * Get the start of the day in the network's timezone, returned as a UTC Date.
 *
 * @param ianaTimeZone - e.g. 'Australia/Brisbane'
 * @param timeZone     - offset string, e.g. '+10:00'
 */
export function getStartOfDay(date: Date, ianaTimeZone: string, timeZone: string): Date {
 ...
 */
const dayStartsMemos: WeakMap<any[], { iana: string; tz: string; result: Date[] }> = new WeakMap();

/**
 * Compute day-start dates from series data for power-mode gridlines.
 *
 * @param data - Series data with `date` or `time` property
 * @param timeZone - offset string
 */
export function getDayStartDates(data: any[], ianaTimeZone: string, timeZone: string): Date[] {
