...
export interface ViewportGesturesConfig {
	/** Read the current viewport */
	viewport: () => { start: number; end: number };
	/**
	 * Commit a new viewport
	 * (write local $state, or forward to `onviewportchange` for provider charts)
	 */
	apply: (start: number, end: number) => void;
	/** Zoom-in floor; omit for unclamped */
	minDurationMs?: (() => number) | null;
	/** Zoom-out ceiling; omit for unclamped */
	maxDurationMs?: (() => number) | null;
	/**
	 * Left-edge floor (no data before this
	...
	/** Pan-gesture start (clear hover, set isPanning) */
	onGestureStart?: () => void;
	/** Pan-gesture end */
	onGestureEnd?: () => void;
	/**
	 ...
	 */
	onSettle?: ((start: number, end: number) => void) | null;
	/** Zoom step for the +/- buttons (default 1.5) */
 ...
 */
export function isViewportPinned(startMs: number, endMs: number, minDateMs: number): boolean {
	return startMs <= minDateMs && endMs >= Date.now() - PINNED_EDGE_TOLERANCE_MS;
}

export function createViewportGestures(config: ViewportGesturesConfig) {
	const {
		viewport,
		apply,
		minDurationMs = null,
	...
	let lastApplied: { start: number; end: number } | null = null;
	...
	function trackStep(start: number, end: number) {
	 ...
	 */
	function handlePan(deltaMs: number) {
	 ...
	 */
	function handleZoom(factor: number, centerMs: number) {
		const { start, end } = viewport();
		const duration = end - start;
		const now = Date.now();
		const floor = minDateMs ? minDateMs() : null;
