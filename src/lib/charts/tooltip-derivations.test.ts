 ...
 */
function makeChart(overrides: Record<string, any> = {}): any {
	return {
		hoverData: undefined,
		focusData: undefined,
		hoverKey: undefined,
		...
		seriesLabels: {},
		seriesColours: {},
		convertAndFormatValue: (v: number) => v.toFixed(1),
		isCategoryChart: false,
		xKey: 'category',
		formatX: (v: any) => String(v),
...
describe('buildSeriesRows', () => {
	...
	it('formats values via chart.convertAndFormatValue', () => {
		const chart = makeChart({
			...
			convertAndFormatValue: (v: number) => `~${v.toFixed(2)}~`
		});
