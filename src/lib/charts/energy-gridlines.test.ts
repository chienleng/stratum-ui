...
const localMidnight = (y: number, m0: number, d: number) => Date.UTC(y, m0, d) - 10 * HOUR;
const localMonthStart = (y: number, m0: number) => Date.UTC(y, m0, 1) - 10 * HOUR;

const rows = (times: number[]) => times.map((time) => ({ time }));

const labels = (g: ReturnType<typeof computeEnergyGridlines>) =>
	g.ticks.map((t) => g.formatTick(t));
...
describe('computeEnergyGridlines — coarse calendar buckets', () => {
	const coarseLabel = (d: Date) => `label:${d.getTime()}`;
