...
export interface TimeFormatPolicy {
	/** Standalone label for one point */
	formatTooltip: (d: Date | number) => string;
	/** Per-bucket axis label, or null to defer to gridline inference */
	bucketTick: ((d: Date | number) => string) | null;
}

/**
 * @param displayInterval - '5m' | '30m' | '1h' | '1d' | '7d' | '1M' | '3M' |
 *        'quarter' | 'season' | 'half' | 'fy' | '1y'
 * @param ianaTimeZone - e.g. 'Australia/Brisbane'
 */
export function getTimeFormatPolicy(
	...
	if (COARSE_BUCKET_INTERVALS.has(displayInterval)) {
		...
		const label =
			displayInterval === '1y'
				? (d: Date | number) => formatYear(d, ianaTimeZone)
				: (d: Date | number) =>
						formatBucketLabel(
							d,
							ianaTimeZone,
							displayInterval as 'quarter' | 'season' | 'half' | 'fy'
						);
