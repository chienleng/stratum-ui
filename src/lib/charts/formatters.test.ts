...
describe('formatTooltipDateTime', () => {
	...
	it('labels coarse calendar buckets', () => {
		const HOUR = 60 * 60 * 1000;
		const local = (y: number, m0: number) => new Date(Date.UTC(y, m0, 1) - 10 * HOUR);
...
describe('applyTimeAxis with empty/partial data', () => {
	const DAY = 24 * HOUR;
	 ...
	 */
	function makeStore(): {
		xTicks: Date[] | undefined;
		xGridlineTicks: Date[] | undefined;
		formatTickX: ((d: any) => string) | undefined;
	} {
	...
	it('produces viewport-spanning energy ticks with no data (1d)', () => {
		...
		const gridlines = store.xGridlineTicks as Date[];
	...
	it('produces viewport-spanning power day ticks with no data', () => {
		...
		const ticks = store.xTicks as Date[];
...
describe('getStartOfDay', () => {
	 ...
	 */
	function intlStartOfDay(date: Date, ianaTimeZone: string, timeZone: string) {
...
describe('getDayStartDates', () => {
	const rows = (times: number[]) => times.map((t) => ({ time: t, date: new Date(t) }));
