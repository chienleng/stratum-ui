/**
 * Energy gridline computation for charts.
 *
...
export interface EnergyGridlines {
	/** Dates where vertical gridlines should be drawn */
	gridlineTicks: Date[];
	/** Midpoint dates where labels are positioned */
	ticks: Date[];
 ...
 *
 * @param visibleData - Array of data points with `.time` property (ms)
 * @param viewStart - Viewport start (ms)
 * @param viewEnd - Viewport end (ms)
 * @param ianaTimeZone - IANA timezone (e.g. 'Australia/Brisbane')
 * @param coarseLabel - When set, the data is in coarse calendar buckets
 *        (season/quarter/half/fy); label one tick per bucket with this fn and
 *        skip the Jan/month-aligned gridline inference.
 */
export function computeEnergyGridlines(
	...
	viewEnd: number,
	ianaTimeZone: string,
	coarseLabel: ((d: Date) => string) | null = null
): EnergyGridlines {
	const dataStarts: Date[] = visibleData.map((d: any) => new Date(d.time));
 ...
 * prior month.)
 */
function bandMidpoints(
	bandStarts: Date[],
	lastBandMs: number
): { midpoints: Date[]; midToStart: Map<number, Date>; midToEnd: Map<number, Date> } {
	const midpoints: Date[] = [];
	const midToStart: Map<number, Date> = new Map();
	const midToEnd: Map<number, Date> = new Map();
