...
import {
	...
	twilightPolygon,
	MERCATOR_LAT_LIMIT,
	type TwilightPolygon
} from './solar-terminator.js';

/** Ray-cast against each world copy because rings unwrap at the antimeridian. */
function contains(ring: [number, number][], lng: number, lat: number): boolean {
...
function ringOf(feature: TwilightPolygon): [number, number][] {
...
describe('twilightPolygon', () => {
	...
	it('shades eastern Australia after sunset while the west is still in daylight', () => {
		// 18:00 in Sydney, but 16:00 in Perth.
	...
	it('leaves both poles in twilight at the equinox for the astronomical band', () => {
		// At the equinox, the astronomical band encloses neither pole.
