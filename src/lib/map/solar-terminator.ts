/**
 * Solar twilight boundaries from low-precision Astronomical Almanac formulas.
 * Accuracy is within a fraction of a degree.
 *
 * Each twilight region is a spherical cap around the antisolar point. This
 * remains stable for every altitude and at the equinoxes.
 */
...
/** GeoJSON polygon of a twilight region, structurally compatible with `GeoJSON.Feature`. */
export interface TwilightPolygon {
	type: 'Feature';
...
/** Solar right ascension and declination, in degrees. */
export function solarPosition(date: Date): { alpha: number; delta: number } {
...
/** Point where the sun is overhead, with longitude in [-180, 180). */
 ...
 *
 * Longitudes stay unwrapped across the antimeridian. Polar caps close along
 * MapLibre's Mercator latitude limit.
 */
export function twilightPolygon(date: Date = new Date(), altitude = 0): TwilightPolygon {
	...
	if (Math.abs(netWrap) > 180) {
		// Close a polar cap along the Mercator limit.
		...
		// Make the closure exact despite floating-point drift.
