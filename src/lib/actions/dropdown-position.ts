...
/**
 * Svelte action that positions a fixed-position dropdown relative to a trigger
 * element. Auto-aligns to the right if the dropdown would overflow the right
 * edge of the viewport.
 */
export const dropdownPosition: Action<HTMLElement, DropdownPositionOpts> = (node, opts) => {
	function update() {
		...
		let align: string | undefined = opts.align;
		if (!align || align === 'left') {
	...
	return {
		update(newOpts: DropdownPositionOpts) {
			opts = newOpts;
			update();
		},
		destroy() {
