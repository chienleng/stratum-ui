<script lang="ts">
	/**
	 * bits-ui-backed port of `ui/options-menu/OptionsMenuItem.svelte`. Identical
	 * props and visuals; rendered as a DropdownMenu.Item so it gains menu
	 * keyboard navigation and highlighting. `closeOnSelect` is disabled to
	 * preserve the original contract where handlers close the menu explicitly
	 * (via the `sections` snippet's `close`). Must be rendered inside the
	 * bits-backed OptionsMenu.
	 */
