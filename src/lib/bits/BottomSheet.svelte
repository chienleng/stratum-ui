<script lang="ts">
	...
	/**
	 * Mobile bottom sheet — bits-ui-backed port of ui/BottomSheet.svelte
	 * (bits-ui Dialog, unportalled so the panel stays anchored inside its
	 * relatively-positioned container). The original drag/snap gesture code is
	 * kept verbatim around Dialog.Content; bits-ui contributes the dialog role
	 * and open-state plumbing only. Deliberately non-modal, as the original: no
	 * backdrop, no focus trap or steal, no scroll lock, no Escape/outside-press
