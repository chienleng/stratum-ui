<script lang="ts">
	/**
	 * bits-ui-backed port of `ui/Switch` — pill-style segmented switch, built
	 * on ToggleGroup (single). Keyboard/ARIA via bits-ui (roving focus with
	 * arrow keys); selection styling keys off data-state instead of
	 * aria-pressed, and deselecting the active segment is guarded so a
	 * selection always remains.
	 */
