<script lang="ts">
	/**
	 * bits-ui-backed port of `forms/Radio`. Same props and visuals; each Radio
	 * renders its own single-item RadioGroup so the bindable `checked` group
	 * value keeps working across instances (keyboard/ARIA via bits-ui — but
	 * arrow keys no longer move between separately rendered Radio instances).
	 */
